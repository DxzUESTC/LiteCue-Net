...
> 基于 Attention 时序建模的轻量级 deepfake 检测框架。
...
LiteCue-Net 是一个面向 deepfake 检测的轻量化视频伪造特征拆解网络。Inter-Clip 模块默认使用轻量 Attention（因果自注意力）实现帧片段间的动态交互，也可切换为 Gated-MLP 以进一步降低参数量。
...
[Stage 2] 全局不一致性分析 — Inter-Clip
    │  ├─ Attention (默认): 动态因果注意力，内容感知交互
    │  └─ Gated-MLP (可选): O(M) 线性混合，轻量高效
    │  (B, M, D)
    ▼
...
| `model.temporal_module` | attention | Inter-Clip 时序建模方式：`attention` (默认, 动态交互) / `gated_mlp` (轻量) |
...
model = LiteCueNet(
    ...
    temporal_module="attention",          # 或 "gated_mlp"
)
