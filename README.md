...
> 基于 Gated-MLP 时序建模的轻量级 deepfake 检测框架。
...
LiteCue-Net 是一个面向 deepfake 检测的轻量化视频伪造特征拆解网络。其核心设计理念是 **"用 MLP 替代 Transformer"**，在保持高检测精度的同时大幅降低参数量和计算开销。
...
[Stage 1] 局部微动捕捉 — Intra-Clip (DW-Conv1D)
    │  捕捉 K 帧内的瞬时运动异常 → 聚合为 Clip Token
    │  (B, M, D)
    ▼
[Stage 2] 全局不一致性分析 — Inter-Clip (Gated-MLP)
    │  M 个片段间全局交互，捕捉长程逻辑矛盾
    │  (B, M, D)
    ▼
...
| 时序建模 | Gated-MLP 替代 Self-Attention，O(M²) → O(M) |
...
│   │       ├── inter_clip.py       # Stage 2: Inter-Clip Gated-MLP
...
├── doc/                            # 文档
│   ├── 使用指南.md                  # 📄 本文档
│   ├── LiteCue-Net 项目整体框架.md
│   ├── LiteCue-Net 使用 Gated-MLP 时序建模的思路.md
│   ├── LiteCue-Net 模型总结.md
│   ├── LiteCue-Net 数据集预处理指南.md
│   ├── LiteCue-Net 跨数据集测试.md
│   └── LiteCue-Net 遮挡增强说明.md
│
├── data/                           # [不提交 Git] 数据集
├── checkpoints/                    # [不提交 Git] 模型权重
├── runs/                           # [不提交 Git] 训练输出
└── logs/                           # [不提交 Git] 日志
```
...
| `model.backbone` | mobilenetv4_conv_small | 骨干网络 |
| `model.use_temporal_diff` | false | 是否启用显式时序差分 |
| `model.use_frequency_branch` | false | 是否启用频域分支 |
...
checkpoint = torch.load("checkpoints/exp/best_model.pth", map_location="cpu")
...
- **data/** 和 **checkpoints/** 目录不会提交到 Git，需自行准备数据与权重。
