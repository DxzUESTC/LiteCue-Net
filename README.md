...
LiteCue-Net 是一个面向 deepfake 检测的轻量化视频伪造特征拆解网络。其核心设计理念是在保持轻量骨干（MobileNetV4-Small）的同时，通过 Transformer 因果自注意力在时序维度捕捉帧片段间的动态不一致性，实现高精度 deepfake 检测。
...
| 阶段 | 参数量 | 说明 |
|------|--------|------|
| 原始版本 | ~3.02M | MobileNetV4 + Gated-MLP + Mean Pool + 固定 HRM（已归档） |
| 当前版本 | ~3.54M | +频域分支 / Transformer / 注意力池化 / 加权融合 / LearnableDecayHRM / 熵正则 |
...
├── requirements-api-gpu.txt        # 推理服务 GPU 依赖
├── LICENSE                         # Apache 2.0
│
├── configs/                        # 配置文件
...
│   ├── config.py                   # 配置管理（含 cuDNN 自动发现）
│   ├── processor.py                # 视频预处理
│   ├── core/                       # Core Layer：人脸检测与对齐（RetinaFace ONNX）
│   │   ├── retinaface_detector.py  #   检测器
│   │   ├── face_align.py           #   仿射对齐
│   │   └── normalize.py            #   标准化
│   ├── model/                      # Model Layer：模型定义
│   │   ├── detector.py             #   LiteCueNet
│   │   ├── backbones/              #   视觉骨干
│   │   └── components/             #   时序组件
...
│
├── data/                           # [不提交 Git] 数据集
├── models/                         # 推理模型（buffalo_l/det_10g.onnx，Git LFS）
├── checkpoints/                    # 训练权重（best_model.pth 已提交 Git）
├── runs/                           # [不提交 Git] 训练输出
├── logs/                           # [不提交 Git] 日志
```
...
| `model.temporal_module` | "attention" | Inter-Clip 时序建模：attention（Transformer）/ gated_mlp（已弃用） |
...
**架构特点：** RetinaFace ONNX 人脸检测（已剥离 InsightFace）、架构参数自动推断、三层抽象（core/model/backend）。

**API 端点：**
- `GET /api/v1/health` — 健康检查
- `POST /api/v1/detect` — 上传视频进行鉴伪，返回伪造概率 + Grad-CAM 热力图

详见 [FastAPI 推理服务文档](doc/LiteCue-Net%20FastAPI%20推理服务.md)。

### 8.2 Gradio Web Demo

项目提供了基于 Gradio 的交互式 Web 界面，支持上传视频/图片进行实时检测。视频判定为伪造时，Demo 会按 clip 伪造概率选取关键帧，并输出叠加 Grad-CAM 的关键帧图片集，用于提示模型关注的可疑区域。
...
- 关键帧热力图是基于 Grad-CAM 的可解释性可视化，表示模型关注区域，不等同于像素级伪造分割掩码。
...
**Q: 如何部署推理服务？**  
A: 参考 [FastAPI 推理服务文档](doc/LiteCue-Net%20FastAPI%20推理服务.md)。服务使用 RetinaFace ONNX 进行人脸检测（已剥离 InsightFace），支持 GPU 加速和 Grad-CAM 热力图输出。
